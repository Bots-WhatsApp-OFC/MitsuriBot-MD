...
global.authFile = `MitsuriBotSession`;
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MitsuriBot-MD', 'Safari', '2.0.0'] : methodCodeQR ? ['MitsuriBot-MD', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '110.0.5585.95'],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./MitsuriBotSession")
...
const directories = ['./MitsuriBotSession/', './BotsWhatsAppOFC/']
...
conn.fakeReply('19145948340@s.whatsapp.net', 'Hola guardian bot md, me eh conectado como nuevo bot.', '0@s.whatsapp.net', '🛡️ 𝙂𝙪𝙖𝙧𝙙𝙞𝙖𝙣𝘽𝙤𝙩-𝙈𝘿 🛡️', '0@s.whatsapp.net')
 await conn.groupAcceptInvite('Bp9kWG8jjoL8j4QLaP0ZZl');
   }
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode;
if (reason == 405) {
await fs.unlinkSync("./MitsuriBotSession/" + "creds.json")
...
setInterval(async () => {
  ...
  const bio = `🎀 𝙈𝙞𝙩𝙨𝙪𝙧𝙞𝘽𝙤𝙩-𝙈𝘿 🎀 / 🥏 𝙒𝙝𝙖𝙩𝙨𝘼𝙥𝙥 🥏 / 𝙏𝙞𝙚𝙢𝙥𝙤: ${uptime}`;
