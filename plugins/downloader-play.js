...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `> 🌻 𝐏𝐎𝐑 𝐅𝐀𝐕𝐎𝐑, 𝐈𝐍𝐆𝐑𝐄𝐒𝐄 𝐄𝐋 𝐍𝐎𝐌𝐁𝐑𝐄 𝐃𝐄𝐋 𝐂𝐎𝐍𝐓𝐄𝐍𝐈𝐃𝐎 𝐐𝐔𝐄 𝐄𝐒𝐓𝐄 𝐁𝐔𝐒𝐂𝐀𝐍𝐃𝐎, 𝐏𝐎𝐑 𝐄𝐉𝐄𝐌𝐏𝐋𝐎: \`#play Rei do Brazil\``;
  try {
...
> ╰◊➣ ${yt_play[0].url}
> ❒═════════════════❒
> Enviando: ${additionalText}, espere
> unos minutos...
`.trim();
    ...
    if (size >= limit_a2) {  
    await conn.sendMessage(m.chat, {text: `> *⬇️ 𝑫𝑬𝑺𝑪𝑨𝑹𝑮𝑼𝑬 𝑺𝑼 𝑨𝑼𝑫𝑰𝑶: ${dl_url}*`}, {quoted: m});
    return;    
    }     
    ...
    const size = fileSizeInMB.toFixed(2);    
    if (size >= limit2) {  
    await conn.sendMessage(m.chat, {text: `> *⬇️ 𝑫𝑬𝑺𝑪𝑨𝑹𝑮𝑼𝑬 𝑺𝑼 𝑽𝑰𝑫𝑬𝑶: ${dl_url}*`}, {quoted: m});
    return;    
    }     
